...
// every session's terminal pane ~50px of height.
//
...
test("session topbar puts identity, tab dock and actions on one row", async ({ page }) => {
  ...
  try {
    ...
    // There is no `tab-terminal`: the terminal is the surface the dock's tabs
    // open a pane *beside*, so it is not one of them.
    for (const id of ["session-tabs", "tab-brainstorm", "tab-files"]) {
    ...
    const tab = await page.getByTestId("tab-files").boundingBox()
    ...
    // The terminal is always mounted, so its pane must start directly under the
    // one row of chrome. Two rows put it at ~80px.
