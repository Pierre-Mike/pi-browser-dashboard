...
import { ClaudeConfigIoLive, ClaudeConfigService, MAX_TEXT_BYTES } from "./claude-config.io"
...
const withLayer = <A, E>(fx: Effect.Effect<A, E, ClaudeConfigService>): Promise<A> => {
  const configLayer = ConfigIoTest({ projectsRoot, claudeConfigDir: globalClaude })
  const projectsLive = Layer.provide(ProjectsIoLive, configLayer)
  const claudeLive = Layer.provide(ClaudeConfigIoLive, Layer.mergeAll(configLayer, projectsLive))
  return Effect.runPromise(Effect.provide(fx, claudeLive))
}

describe("ClaudeConfigIo readGlobal", () => {
...
describe("ClaudeConfigIo readProject", () => {
...
describe("ClaudeConfigIo readSkill", () => {
...
describe("ClaudeConfigIo read size cap", () => {
  let capRoot: string
  ...
  const withCapLayer = <A, E>(fx: Effect.Effect<A, E, ClaudeConfigService>): Promise<A> => {
    const configLayer = ConfigIoTest({ projectsRoot: capProjectsRoot, claudeConfigDir: capRoot })
    const projectsLive = Layer.provide(ProjectsIoLive, configLayer)
    const claudeLive = Layer.provide(ClaudeConfigIoLive, Layer.mergeAll(configLayer, projectsLive))
    return Effect.runPromise(Effect.provide(fx, claudeLive))
  }
