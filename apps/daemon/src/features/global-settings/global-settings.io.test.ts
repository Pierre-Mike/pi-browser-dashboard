...
import {
  ...
  GlobalSettingsIoLive,
  GlobalSettingsService,
} from "./global-settings.io"
...
const layer = () => GlobalSettingsIoLive.pipe(Layer.provide(ConfigIoTest({ claudeConfigDir: dir })))

const run = <A>(eff: Effect.Effect<A, never, GlobalSettingsService>): Promise<A> =>
  Effect.runPromise(Effect.provide(eff, layer()))

describe("GlobalSettingsIoLive", () => {
