...
// an in-memory ProjectsIoTest fixture (so resolveProjectDir finds the path).
let root: string
let projA: Project
let projB: Project
...
const attempt = <A, E>(proj: Project, eff: Effect.Effect<A, E, PidAppsService>) =>
  Effect.runPromise(
    Effect.either(Effect.provide(eff, Layer.provide(PidAppsIoLive, ProjectsIoTest([proj])))),
  )
...
describe("PidAppsIoLive.listApps", () => {
...
describe("PidAppsIoLive.resolveAsset", () => {
...
describe("PidAppsIoLive.createApp", () => {
...
describe("PidAppsIoLive: specs/-sourced apps", () => {
