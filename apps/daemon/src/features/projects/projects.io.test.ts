...
import { ProjectsIoLive, ProjectsService } from "./projects.io"
...
const withLayer = <A, E>(fx: Effect.Effect<A, E, ProjectsService>): Promise<A> => {
  const layer = Layer.provide(ProjectsIoLive, ConfigIoTest({ projectsRoot: root }))
  return Effect.runPromise(Effect.provide(fx, layer))
}

describe("ProjectsIo listDir", () => {
...
describe("ProjectsIo list ordering by last commit", () => {
  let gitRoot: string
  ...
  it("ranks a freshly-committed git repo ahead of a recently-touched plain dir", async () => {
    const layer = Layer.provide(ProjectsIoLive, ConfigIoTest({ projectsRoot: gitRoot }))
...
describe("ProjectsIo resolveRaw", () => {
