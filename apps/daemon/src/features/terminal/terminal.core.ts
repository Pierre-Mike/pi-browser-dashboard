...
// quit claude and have the pane disappear instead of staring at a corpse. The
// caller writes this to a file and passes the path to projectZellijCommand —
// zellij's `-n` (--new-session-with-layout) flag only accepts a file path.
...
// behavior is misleading: with `--session` the layout is treated as "add tabs
// to the existing session", and when no session by that name exists zellij
// prints "Session 'NAME' not found. The following sessions are active: …" and
// exits 0 without creating anything — so the per-project session was never
// actually created on first open. `-n FILE` always starts a new session.
